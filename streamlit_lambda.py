...
# Tabs
tab1, tab2, tab3, tab4 = st.tabs(["🔍 Encontrar Parceiros", "👥 Grupos Formados", "📋 Pessoas Cadastradas", "🗑️ Gerenciar"])
...
with tab4:
    ...
    if passengers:
        ...
        if st.button("🗑️ Remover Passageiro", type="secondary"):
        ...
        if st.button("🗑️ Limpar Todos os Passageiros", type="secondary"):
            if call_api("DELETE", "/passengers"):
                st.success("Todos os passageiros removidos!")
                st.rerun()
        
        if st.button("🗑️ Limpar Grupos Formados", type="secondary"):
            st.session_state.formed_groups = []
...
if "LAMBDA_API_URL" not in st.secrets:
    st.warning("⚠️ Configure LAMBDA_API_URL nos secrets do Streamlit")